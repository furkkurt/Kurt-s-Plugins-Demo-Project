...
(() => {
  ...
  // Force idle to frame 0
  const _resetPattern = Game_CharacterBase.prototype.resetPattern;
  Game_CharacterBase.prototype.resetPattern = function() {
    _resetPattern.call(this);
    if (this._cframes && this._cframes > 3) {
      this._pattern = 0;
    }
  };

  // Walking animation: skip frame 0
  const _updatePattern = Game_CharacterBase.prototype.updatePattern;
  Game_CharacterBase.prototype.updatePattern = function() {

    // If stopped, let Galv / engine handle idle
    if (!this.isMoving()) {
      return;
    }

    // Only custom-handle multi-frame characters
    if (this._cframes && this._cframes > 3) {
      this._pattern++;
      ...
      // Loop walking frames only
      if (this._pattern >= this._cframes) {
        this._pattern = 1;
      }
...
const _update = Game_CharacterBase.prototype.update;
  Game_CharacterBase.prototype.update = function() {
    ...
    // Only snap to idle when truly stopped (not between steps)
    if (
      this._cframes &&
      this._cframes > 3 &&
      !this.isMoving() &&
      this._stopCount === 1   // <-- THIS IS THE KEY
    ) {
      this._pattern = 0;
    }
  };
